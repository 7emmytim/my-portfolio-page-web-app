import img3 from "../../images/3.jpg"
import booking from "../../images/svg/booking.svg"
import hiking from "../../images/svg/hiking.svg"
import coder4 from "../../images/svg/coder4.svg"
import coder from "../../images/svg/coder.svg"
import contact from "../../images/svg/contact.svg"
import notepad from "../../images/notepad.png"
import covidB from "../../images/covidB.png"
import quiz from "../../images/quiz.png"
import airdreams from "../../images/airdreams.jpg"

export const homeObjOne = {
    lightBg: true,
    imgStart: false,
    textWrapperAlign: true,
    lightTopLine: true,
    lightTextDesc: false,
    lightText: false,
    topLine: "My Portfolio Page",
    headline: "Hello, It's Timilehin, Your Favourite Developer",
    description: "I am a Front-End Developer (B.Sc. Geophysics). I build Websites and apps with technologies like React Js, Wordpress and Others",
    start: "",
    bRadius: true,
    img: img3
}

export const homeObjTwoB = {
    lightBg: true,
    imgStart: true,
    textWrapperAlign: false,
    lightTopLine: false,
    lightTextDesc: false,
    lightText: false,
    topLine: "Get A Quality Website Today!",
    headline: "About Timilehin",
    description: "I build Apps for Web and Mobile platforms. I am an Open-Minded Learner, Problem-Solver, Creative Developer and a Geophysicist.",
    start: "",
    bRadius: true,
    img: img3    
}

export const homeObjThree = {
    lightBg: true,
    imgStart: true,
    textWrapperAlign: true,
    lightTopLine: false,
    lightTextDesc: false,
    lightText: false,
    topLine: "Get Your Website Done Right!",
    headline: "Book With Timilehin",
    // description: "Contact me with your job.",
    btnLink: "/contact",
    btnShow: true,
    buttonLabel: "Book Now",
    start: "",
    img: booking
}

export const homeObjThreeB = {
    lightBg: true,
    imgStart: false,
    textWrapperAlign: true,
    lightTopLine: false,
    lightTextDesc: false,
    lightText: false,
    topLine: "Message Me With Your Job",
    headline: "Contact Timilehin",
    description: "Fill the form below to reach out to me with your jobs.",
    start: "",
    img: hiking
}

export const homeObjFour = {
    lightBg: true,
    imgStart: false,
    textWrapperAlign: false,
    lightTopLine: false,
    lightTextDesc: false,
    lightText: false,
    topLine: "Check My Recent Works",
    headline: "Projects & Recent Works",
    description: "My works are responsive (fits every screen size), highly functional, simple and easy to use",
    btnShow: true,
    buttonLabel: "See Works",
    btnLink: "/projects",
    start: "",
    img: coder4
}

export const homeObjFourB = {
    lightBg: true,
    imgStart: true,
    textWrapperAlign: false,
    lightTopLine: false,
    lightTextDesc: false,
    lightText: false,
    // topLine: "Github Page - https://github.com/7emmytim",
    headline: "Projects & Recent Works",
    description: "You can visit my Github Page from the contact page",
    start: "",
    img: coder   
}

export const homeObjFourC = {
    lightBg: true,
    imgStart: false,
    textWrapperAlign: false,
    lightTopLine: false,
    lightTextDesc: false,
    lightText: false,
    // topLine: "https://notepad-clone.web.app/",
    headline: "NOTEPAD APP",
    description: "Progressive Web App built using React Js. It can add, edit and delete note. Your note is saved locally to your device which means you can access it anytime and it is privy to you.",
    start: "",
    hImage: true,
    img: notepad
}

export const homeObjFourD = {
    lightBg: true,
    imgStart: true,
    textWrapperAlign: false,
    lightTopLine: false,
    lightTextDesc: false,
    lightText: false,
    // topLine: "http://covidtestapp.web.app/",
    headline: "COVID-19 APP",
    description: "App built using Vue Js. You can view results from different countries, you can also read the covid-19 guidlines and safety measures as prescribed by health organizations.",
    start: "",
    hImage: true,
    img: covidB   
}

export const homeObjFourE = {
    lightBg: true,
    imgStart: false,
    textWrapperAlign: false,
    lightTopLine: false,
    lightTextDesc: false,
    lightText: false,
    // topLine: "https://7emmytim.github.io/my-quiz-app",
    headline: "QUIZ APP",
    description: "App built using Vanilla Js. For taking quiz from different categories.",
    start: "",
    hImage: true,
    img: quiz    
}

export const homeObjFourF = {
    lightBg: true,
    imgStart: true,
    textWrapperAlign: false,
    lightTopLine: false,
    lightTextDesc: false,
    lightText: false,
    // topLine: "Airdreams standard trampolining",
    headline: "AIRDREAMS WEBSITE",
    description: "Landing page built with HTML, CSS and JS.",
    start: "",
    hImage: true,
    img: airdreams
}

export const homeObjFive = {
    lightBg: true,
    imgStart: true,
    textWrapperAlign: true,
    lightTopLine: false,
    lightTextDesc: false,
    lightText: false,
    topLine: "Leave A Message For Me",
    headline: "Contact Timilehin",
    // description: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Id quae ad culpa aut dolore esse quo corrupti officiis nesciunt",
    btnShow: true,
    buttonLabel: "Message Me",
    btnLink: "/contact",
    start: "",
    img: contact
}